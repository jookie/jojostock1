...
def execute_trade(balance, holdings, action, row, trade_log):
    if action == "BUY":
        ...
        trade_log.append({
            "Action": "SELL", "Symbol": symbol, "Date": row["Date"], "Time": row["Time"],
            "Price": row["Price"], "Score": row["Sentiment Score"], "Headline": row["Headline"]
        })
        balance += row["Price"]
        holdings -= 1
...
if start_date >= end_date:
    ...
    try:
        fetched_news = api.get_news(symbol=symbol, start=start_date.strftime('%Y-%m-%d'), end=end_date.strftime('%Y-%m-%d'))
        parsed_data = []

        for article in fetched_news:
            sentiment_scores = estimate_sentiment(article.headline)
            positive_score = float(sentiment_scores[0])
            parsed_data.append({
    ...
    if parsed_data:
        ...
        st.json({"Average Score": avg_score})
        ...
        backtest_col1, backtest_col2, backtest_col3 = st.columns([1, 1, 1])
        ...
        if backtest_start_date >= backtest_end_date:
            ...
            try:
                ...
                for article in backtest_news:
                    sentiment_scores = estimate_sentiment(article.headline)
                    positive_score = float(sentiment_scores[0])
                    price_data = api.get_bars(symbol, "1Day", start=article.created_at.strftime('%Y-%m-%d'), end=article.created_at.strftime('%Y-%m-%d')).df
                ...
                for _, row in backtest_df.iterrows():
                    if row["Sentiment Score"] > sentiment_threshold:
                        balance, holdings = execute_trade(balance, holdings, "BUY", row, trade_log)
                    elif row["Sentiment Score"] < -sentiment_threshold and holdings > 0:
                        balance, holdings = execute_trade(balance, holdings, "SELL", row, trade_log)
