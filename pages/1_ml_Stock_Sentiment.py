...
def alpaca_history(alpaca_API_KEY, alpaca_API_SECRET, ticker, start_date, end_date):
    """Fetch historical stock data from Alpaca with user-selected dates."""
...
if ticker:
    ...
    stock_data = alpaca_history(ALPACA_API_KEY, ALPACA_API_SECRET, ticker, start_date, end_date)
    ...
    if ticker:
        ...
        if stock_data:
            ...
            st.write(stock_data)  # Display raw data
        else:
            st.warning("No stock data retrieved. Try adjusting the date range or ticker.")
