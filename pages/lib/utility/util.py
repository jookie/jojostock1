...
def get_ticker_start_end_date():
    ...
    st.write(f"Running sentiment analysis for {ticker} from {start_date} to {end_date} with {cash_at_risk * 100}% cash at risk...")
    ...
    return ticker, start_date, end_date, cash_at_risk
...
def get_baseline2(ticker: str, start: str, end: str) -> pd.DataFrame:
    ...
    try:
        ...
        if bars.empty:
            raise ValueError("No baseline data returned from Alpaca.")
        ...
        # Ensure date column is tz-naive to avoid tz_localize error
        bars["date"] = bars["date"].dt.tz_localize(None)

        bars.reset_index(drop=True, inplace=True)
        return bars[["date", "close"]].rename(columns={"close": "close"})

    except Exception as e:
        raise ValueError(f"Failed to fetch baseline data: {e}")
...
def alpaca_hist(tickers, start_date, end_date):
    ...
    for ticker in tickers:
        ...
        try:
            ...
            data_frames.append(bars[["date", "tic", "open", "high", "low", "close", "volume"]])
    ...
    if data_frames:
        ...
        combined_df.to_csv("alpaca_6_months_data.csv", index=False)
...
def load_and_plot_stock_data(ticker, start_date, end_date):
